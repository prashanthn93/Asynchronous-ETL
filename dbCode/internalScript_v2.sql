...
#-- The code is for Completed days ----------------------__# 
...
CREATE or replace VIEW `Requirement_CardData_fixversion` AS
/*#----- The code is for Groomed User Stories ------ # */
 SELECT
	XX.project_id AS kpi_project_id,
	CONCAT('Day ', XX.day_count) AS TrackerChange ,
...
FROM
	(
	SELECT
		project_id, dt, day_count,fix_version,  Round(100 *(count(DISTINCT CASE WHEN status_end_day IN (10000, 3, 11708, 11709, 11710, 10205, 10403) THEN issue_id END ) / count(DISTINCT issue_id)), 2 ) AS groomed_perc
	FROM
		project_issue_daily_status_intermediate
	GROUP BY
		1, 2, 3, 4) XX
LEFT JOIN (
	...
	XX.project_id = YY.project_id
	AND XX.day_count - 1 = YY.day_count
	and XX.fix_version = YY.fix_version
UNION
#-- Code for Total Stories ----------------- # 
 SELECT
	XX.project_id AS kpi_project_id,
...
FROM
	(
	SELECT
		project_id, dt, day_count,fix_version,  count(DISTINCT issue_id) AS total_stories
	FROM
		project_issue_daily_status_intermediate
	GROUP BY
		1, 2, 3,4 ) XX
...
FROM
	(
	SELECT
		pbt.*, mi.fix_version, COALESCE(mi.issues,
		0) AS issues
	FROM
		proj_base_table pbt
	LEFT JOIN (
		SELECT
			project_id, CAST(created AS date) AS created_date,fix_version,  count(DISTINCT issue_id ) AS issues
		FROM
			master_issue
		WHERE
			issue_type_id NOT IN (10000)
			GROUP BY 1 , 2, 3) mi ON
		pbt.project_id = mi.project_id
		AND pbt.dt = mi.created_date ) XX
LEFT JOIN (
	SELECT
...
FROM
	(
	SELECT
		project_id, dt, day_count, fix_version, count(DISTINCT CASE WHEN status_end_day  IN (10000, 3, 11708, 11709, 11710, 10205, 10403) THEN issue_id END ) AS to_do
	FROM
		project_issue_daily_status_intermediate
	GROUP BY
		1, 2, 3,4 ) XX
...
FROM
	(
	SELECT
		project_id, dt, day_count,fix_version, count(DISTINCT CASE WHEN status_end_day IN (10104) THEN issue_id END ) AS blocked
	FROM
		project_issue_daily_status_intermediate
	GROUP BY
		1, 2, 3,4 ) XX
...
FROM
	(
	SELECT
		mi.project_id,fix_version ,Round(SUM(COALESCE(mcl.change_item_to, mi.time_original_estimate, 0)) / 3600, 2) AS original
	FROM
		master_issue mi
	LEFT JOIN (
		SELECT
...
RIGHT JOIN (
	SELECT
		mi.project_id,fix_version,  Round(SUM(COALESCE(mcl.change_item_to, mi.time_original_estimate, 0)) / 3600, 2) AS new_time
	FROM
		master_issue mi
	LEFT JOIN (
		SELECT
...
select
	...
	proj.epic_end_date,
	ct.dt,
	rank() over(PARTITION BY epic_name order by dt asc) as day_count
from
	(
	select
		project_id,
...
/* Fix version Ads for epic summary and estimate summary */

CREATE or replace
VIEW `Requirement_EpicSummary_fixversion` AS
select
	ebl.project_id as 'project_id',
	ebl.epic_name as 'Epic_name',
	COALESCE(epic_name.epic_name_text, ebl.epic_name) as 'Epic_name_text',
